...
// check command line inputs
  ...
  if ((on = option_match(argc,argv,"-noest")) > -1)
  {
    ...
    N_warn++;
    warning << N_warn << " SS3 exited with -noest, use -maxI <maxphase> instead" << endl;
    exit(1);
  }
...
// -999 means to stop reading items for the list
    ...
    {
      if (reportdetail_list[j](1) == -100)
        rd_background = 0;
      if (reportdetail_list[j](1) == -101)
        rd_background = 1;
      if (reportdetail_list[j](1) == -102)
        rd_background = 2;
    }
  }
  else
...
//  set background set of picked reports; then set custom if reportdetail==3
  ...
  if (reportdetail == 3)
  {
    for (unsigned j = 0; j <= reportdetail_list.size()-2; j++)
    {
      ...
      {
        N_warn++;  
        warning << N_warn << " " << "custom report number: " << reportdetail_list[j](1) << " is out of range and ignored" << endl;
      }
    }
  }
 ...
 LOCAL_CALCS
  {
    ...
    {
      N_warn++;
      warning << N_warn << " " << "NOTE: new feature for multiyr F_std reporting, be sure STD reporting covers all years from styr to endyr" << endl;
    }
    ...
    if (tempin == 999.)  // finish read in 3.24 format for ss_trans
    {
      ...
      {
        N_warn++; 
        warning << N_warn << " ss_trans does not read the PAR file; readparfile set to 0" << endl << endl;
        readparfile = 0;
      }
    }
    else   // reading in 3.30 format
    {
      ...
      *(ad_comm::global_datafile) >> ALK_tolerance;
      ...
      {
         N_warn++;
         warning << N_warn << " Error: ALK tolerance must be between 0.0 and 0.1" << endl;
         cout << "Error: ALK_tolerance must be between 0.0 and 0.1: " << ALK_tolerance << endl;
         exit(1);
      }
      ...
      if (ender == 0) 
      {
        *(ad_comm::global_datafile) >> tempin;
        ...
        else
        {
          ...
          warning << N_warn << " " << "starter.ss has extra input lines; check echoinput to verify read" << endl;
       	  echoinput << endl << "starter.ss should have read 3.30 here; it read: " << tempin << endl;
       	  exit(1);
       	}
      }
    }
...
!!//  SS_Label_Info_1.2.1 #Set up a dummy datum for use when max phase = 0
 ...
 LOCAL_CALCS
  ...
  }  // just to have something in scope
